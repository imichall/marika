<template>
  ...
  >
    <div
      class="flex items-center px-4 py-4 gap-3 text-slate-900 font-semibold dark:text-white border-b border-slate-200 dark:border-slate-700"
    >
      <Icon
        name="mdi:music-clef-treble"
        class="text-3xl text-red-600 shrink-0"
      />
    ...
    <div
      class="px-4 py-4 border-t border-slate-200 space-y-2 dark:border-slate-700"
    >
      <NuxtLink
        v-if="canAccessAdmin"
        to="/admin"
...
const filteredLinks = computed(() =>
  props.links.filter((link) =>
    link.permission ? link.permission === props.userRole : true
  )
);
